...
static struct dd_flags g_flags[] = {{"append", O_APPEND},       {"direct", O_DIRECT},
                                    {"directory", O_DIRECTORY}, {"dsync", O_DSYNC},
                                    {"noatime", O_NOATIME},     {"noctty", O_NOCTTY},
                                    {"nofollow", O_NOFOLLOW},   {"nonblock", O_NONBLOCK},
                                    {"sync", O_SYNC},           {NULL, 0}};
...
static void _dd_bdev_seek_hole_done(struct spdk_bdev_io *bdev_io, bool success,
                                    void *cb_arg);

static void dd_cleanup_bdev(struct dd_target io) {
...
static void dd_exit(int rc) {
    ...
    if (g_job.input.type == DD_TARGET_TYPE_FILE ||
        g_job.output.type == DD_TARGET_TYPE_FILE) {
#ifdef SPDK_CONFIG_URING
        if (g_opts.aio == false) {
...
static void dd_show_progress(bool finish) {
    ...
    if (finish) {
        ...
        milliseconds =
            spdk_max(1, TIMESPEC_TO_MS(time_now) - TIMESPEC_TO_MS(g_job.start_time));
    ...
    printf("\33[2K\rCopying: %" PRIu64 "/%" PRIu64 " [%sB] (%s%" PRIu64 " %sBps)",
           g_job.total_bytes / size_unit, g_job.copy_size / size_unit, size_unit_str,
           speed_type, speed / speed_unit, speed_unit_str);
    fflush(stdout);
}

static int dd_status_poller(void *ctx) {
    dd_show_progress(false);
    return SPDK_POLLER_BUSY;
}

static void dd_finalize_output(void) {
    off_t curr_offset;
    int rc = 0;
...
#ifdef SPDK_CONFIG_URING
static void dd_uring_submit(struct dd_io *io, struct dd_target *target, uint64_t length,
                            uint64_t offset) {
    struct io_uring_sqe *sqe;

    sqe = io_uring_get_sqe(&g_job.u.uring.ring);
    if (io->type == DD_READ || io->type == DD_POPULATE) {
        io_uring_prep_read_fixed(sqe, target->u.uring.idx, io->buf, length, offset,
                                 io->idx);
    } else {
        io_uring_prep_write_fixed(sqe, target->u.uring.idx, io->buf, length, offset,
                                  io->idx);
    }
...
static void _dd_write_bdev_done(struct spdk_bdev_io *bdev_io, bool success,
                                void *cb_arg) {
...
static void dd_target_write(struct dd_io *io) {
    ...
    if (target->type == DD_TARGET_TYPE_FILE) {
        ...
        rc = spdk_bdev_write(target->u.bdev.desc, target->u.bdev.ch, io->buf,
                             write_offset, length, _dd_write_bdev_done, io);
    }
...
static void _dd_read_bdev_done(struct spdk_bdev_io *bdev_io, bool success, void *cb_arg) {
    struct dd_io *io = cb_arg;
...
static void dd_target_read(struct dd_io *io) {
    ...
    if (target->type == DD_TARGET_TYPE_FILE) {
        ...
        rc = spdk_bdev_read(target->u.bdev.desc, target->u.bdev.ch, io->buf, io->offset,
                            io->length, _dd_read_bdev_done, io);
    }
...
static void _dd_target_populate_buffer_done(struct spdk_bdev_io *bdev_io, bool success,
                                            void *cb_arg) {
    struct dd_io *io = cb_arg;
...
static void dd_target_populate_buffer(struct dd_io *io) {
    ...
    if (target->type == DD_TARGET_TYPE_FILE) {
        ...
        rc = spdk_bdev_read(target->u.bdev.desc, target->u.bdev.ch, io->buf, write_offset,
                            length, _dd_target_populate_buffer_done, io);
    }
...
static off_t dd_file_seek_data(void) {
...
static off_t dd_file_seek_hole(void) {
...
static void _dd_bdev_seek_data_done(struct spdk_bdev_io *bdev_io, bool success,
                                    void *cb_arg) {
    struct dd_io *io = cb_arg;
...
static void _dd_bdev_seek_hole_done(struct spdk_bdev_io *bdev_io, bool success,
    ...
    if (next_hole_offset_blocks == UINT64_MAX) {
        ...
        io->length =
            spdk_min((uint64_t)g_opts.io_unit_size,
...
static void dd_complete_poll(struct dd_io *io) {
    assert(g_job.outstanding > 0);
    g_job.outstanding--;
...
#ifdef SPDK_CONFIG_URING
static int dd_uring_poll(void *ctx) {
...
static int dd_aio_poll(void *ctx) {
...
static int dd_open_file(struct dd_target *target, const char *fname, int flags,
                        uint64_t skip_blocks, bool input) {
    int *fd;
...
static void dd_bdev_event_cb(enum spdk_bdev_event_type type, struct spdk_bdev *bdev,
                             void *event_ctx) {
    SPDK_NOTICELOG("Unsupported bdev event: type %d\n", type);
}

static int dd_open_bdev(struct dd_target *target, const char *bdev_name,
    ...
    rc =
        spdk_bdev_open_ext(bdev_name, true, dd_bdev_event_cb, NULL, &target->u.bdev.desc);
    if (rc < 0) {
    ...
    target->total_size =
        spdk_bdev_get_num_blocks(target->u.bdev.bdev) * target->block_size;

    g_opts.queue_depth = spdk_min(
        g_opts.queue_depth, (target->total_size / g_opts.io_unit_size) - skip_blocks + 1);
...
static int parse_flags(char *file_flags) {
    char *input_flag;
    int flags = 0;
    int i;
    bool found = false;
...
static void *dd_uring_init(void *arg) {
    struct dd_uring_init_ctx *ctx = arg;

    ctx->rc = io_uring_queue_init(g_opts.queue_depth * 2, &g_job.u.uring.ring,
                                  ctx->io_uring_flags);
    return ctx;
}

static int dd_register_files(void) {
    int fds[2];
    unsigned count = 0;
...
static int dd_register_buffers(void) {
    struct iovec *iovs;
    int i, rc;
...
static void dd_run(void *arg1) {
    ...
    if (g_opts.input_file) {
        ...
        if (dd_open_file(&g_job.input, g_opts.input_file, flags, g_opts.input_offset,
                         true) < 0) {
    ...
    /* We cannot check write size for input files because /dev/zeros, /dev/random, etc
     * would not work. We will handle that during copying */
    if (g_opts.input_bdev && g_job.input.pos > g_job.input.total_size) {
        SPDK_ERRLOG("--skip value too big (%" PRIu64 ") - only %" PRIu64
                    " blocks available in input\n",
    ...
    if (g_opts.io_unit_count != 0 && g_opts.input_bdev &&
        ...
        SPDK_ERRLOG("--count value too big (%" PRIu64 ") - only %" PRIu64
                    " blocks available from input\n",
                    g_opts.io_unit_count,
                    (g_job.input.total_size - g_job.input.pos) / g_opts.io_unit_size);
        dd_exit(-ENOSPC);
        return;
    }
    ...
    if (g_opts.output_file) {
        ...
        if (dd_open_file(&g_job.output, g_opts.output_file, flags, g_opts.output_offset,
                         false) < 0) {
        ...
        if (g_job.output.pos > g_job.output.total_size) {
            SPDK_ERRLOG("--seek value too big (%" PRIu64 ") - only %" PRIu64
                        " blocks available in output\n",
                        g_opts.output_offset,
                        g_job.output.total_size / g_opts.io_unit_size);
            dd_exit(-ENOSPC);
            return;
        }

        if (g_opts.io_unit_count != 0 &&
            write_size + g_job.output.pos > g_job.output.total_size) {
            SPDK_ERRLOG("--count value too big (%" PRIu64 ") - only %" PRIu64
                        " blocks available in output\n",
                        g_opts.io_unit_count,
                        (g_job.output.total_size - g_job.output.pos) /
                            g_opts.io_unit_size);
            dd_exit(-ENOSPC);
            return;
        }
    ...
    if ((g_job.output.block_size > g_opts.io_unit_size) ||
        ...
        SPDK_ERRLOG("--bs value cannot be less than input (%d) neither output (%d) "
                    "native block size\n",
    ...
    for (i = 0; i < g_opts.queue_depth; i++) {
        g_job.ios[i].buf =
            spdk_malloc(g_opts.io_unit_size, 0x1000, NULL, 0, SPDK_MALLOC_DMA);
    ...
    if (g_opts.input_file || g_opts.output_file) {
        ...
        if (g_opts.aio == false) {
            ...
            int flags = parse_flags(g_opts.input_file_flags) &
                        parse_flags(g_opts.output_file_flags);

            ctx.io_uring_flags = IORING_SETUP_SQPOLL;
            if ((flags & O_DIRECT) != 0 && dd_is_blk(g_job.input.u.uring.fd) &&
            ...
            /* Initialized uring kernel threads inherit parent process CPU mask, to avoid
             * conflicting with SPDK cores initialize uring without any affinity. */
    ...
    g_job.status_poller = SPDK_POLLER_REGISTER(
        dd_status_poller, NULL, STATUS_POLLER_PERIOD_SEC * SPDK_SEC_TO_USEC);
